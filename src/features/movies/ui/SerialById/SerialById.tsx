              ...
              <div className={s.container}>
                <div className={s.title}>
                  ...
                  <div className={s.actors}>
                    <span>crew: </span>
                    {cast?.crew.slice(0, 3).map((a: Crew) => <span key={a.name}>{a.name}</span>)}
                  </div>
                  <div className={s.actors}>
                    <span>actors: </span>
                    {cast?.cast.slice(0, 3).map((a: Cast) => <span key={a.name}>{a.name}</span>)}
                  </div>
                        ...
                        {!similar?.results.length
                          ...
                          : similar.results.map(movie => (
                              <SwiperSlide key={movie.id}>
                                <MovieCard movie={movie} pathname={'movies'} />
                              </SwiperSlide>
                            ))}
                      </Slider>
                    ) || '',
      ...
      <Modal className={s.modalOverlay} disabled={false} onClose={setOpenShare} open={openShare}>
        <div className={s.share}>
          ...
          <div className={s.shareIcons}>
            <EmailShareButton url={`${pathname}/movies/${id}`}>
              <EmailIcon round />
            </EmailShareButton>
            <LinkedinShareButton url={`${pathname}/movies/${id}`}>
              <LinkedinIcon round />
            </LinkedinShareButton>
            <TelegramShareButton url={`${pathname}/movies/${id}`}>
              <TelegramIcon round />
            </TelegramShareButton>
            <TwitterShareButton url={`${pathname}/movies/${id}`}>
              <TwitterIcon round />
            </TwitterShareButton>
            <ViberShareButton url={`${pathname}/movies/${id}`}>
              <ViberIcon round />
            </ViberShareButton>
            <WhatsappShareButton url={`${pathname}/movies/${id}`}>
              <WhatsappIcon round />
            </WhatsappShareButton>
          </div>
